        ...
        <div id="popupModal" class="modal fade show" tabindex="-1" aria-modal="true" role="dialog" style="display:block; background:rgba(30,30,30,0.35); z-index:2000;">
            <div class="modal-dialog modal-lg modal-dialog-centered" style="max-width:950px;">
                <div class="modal-content" style="border-radius:18px; overflow:hidden; position:relative;">
                    ...
                    @if($popup->link)
                        <a href="{{ $popup->link }}" target="_blank" style="display:block;">
                            <img src="{{ asset('storage/' . $popup->image_path) }}" alt="PopUp" style="width:100%; max-width:900px; display:block; margin:0 auto;">
                        </a>
                    @else
                        <img src="{{ asset('storage/' . $popup->image_path) }}" alt="PopUp" style="width:100%; max-width:900px; display:block; margin:0 auto;">
                    @endif
                    @if($popup->message)
                        <div style="text-align:center; font-size:1.18rem; font-weight:600; color:#1976d2; margin:0.7rem 0 1.2rem 0;">{{ $popup->message }}</div>
                    @endif
                </div>
            </div>
        </div>
